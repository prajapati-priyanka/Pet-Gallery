...
const SecondaryBtn = styled.button<{ $active?: boolean }>`
  ...
  border: 1px solid
    ${({ theme, $active }) =>
      $active ? theme.colors.primaryMid : theme.colors.border};
  border-radius: ${({ theme }) => theme.radii.md};
  transition: all 150ms ease;
  cursor: pointer;
  &:hover {
...
const RelImgWithFallback = ({ src, alt }: { src: string; alt: string }) => {
  ...
  return (
    <RelImgWrap>
      {err || !src ? (
        <RelImgFallback>🐾</RelImgFallback>
      ) : (
        <RelImg src={src} alt={alt} onError={() => setErr(true)} />
      )}
    </RelImgWrap>
  );
};
...
export default function DetailPage() {
  ...
  if (loading)
    return (
      <Page>
        <div
          style={{ padding: "80px 40px", textAlign: "center", color: "#aaa" }}
        >
          Loading…
        </div>
      </Page>
    ...
    return (
      <Page>
        <NotFound>
          ...
          <Link
            to="/gallery"
            style={{ color: "#D85A30", marginTop: 16, display: "inline-block" }}
          >
            ← Back to gallery
          </Link>
        </NotFound>
    ...
    <Page>
      ...
      <Content>
        ...
        <InfoPanel>
          ...
          <Actions>
            <PrimaryBtn onClick={() => downloadSingle(pet)}>
              ↓ Download image
            </PrimaryBtn>
